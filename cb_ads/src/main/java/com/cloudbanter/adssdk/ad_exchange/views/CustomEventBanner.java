...
public abstract class CustomEventBanner {
  ...
  public abstract void loadBanner(Context context,
                                  CustomEventBanner.CustomEventBannerListener
  ...
  public void addViewEvent(View bannerView) {
    ...
    if (ea != null) {
      Log.i(TAG, "Event sent from source: " + source);
      ea.addView(source);
    }
  }
  
  protected void handleFavorites(View bannerView) {
    if (bannerView instanceof ViewGroup) {
      BannerGrabber.getInstance().handleFavourites((ViewGroup) bannerView);
    }
  }
  
  /**
   * This source is intended to be returned, using one of the {@link BannerGrabber} constants, such
   * as, {@link BannerGrabber#SOURCE_FACEBOOK_AD}
   *
